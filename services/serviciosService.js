...
class ServiciosService {
  ...
  async calcularPrecioConServicios(estudianteUnidadId) {
    const estudianteUnidad = await EstudianteUnidad.findByPk(estudianteUnidadId, {
      include: [
        { model: Unidad, as: 'unidad', required: true },
        {
          model: Servicio,
          as: 'servicios',
          through: { attributes: ['fecha_agregado', 'precio_snapshot', 'estado', 'fecha_inicio', 'fecha_fin'] }
        }
      ]
    });

    if (!estudianteUnidad) throw new Error('Asignación no encontrada');
    if (!estudianteUnidad.unidad) throw new Error('Unidad asociada no encontrada');
    ...
    for (const servicio of serviciosVigentes) {
      asignacionConPrecio = new ServicioDecorator(asignacionConPrecio, servicio, servicio.estudiante_unidad_servicio);
    }
  ...
  async agregarServicioAAsignacion(estudianteUnidadId, servicioId) {
    ...
    const servicio = await Servicio.findOne({ where: { id: servicioId, activo: true } });
    if (!servicio) throw new Error('Servicio no encontrado o inactivo');
    if (servicio.es_base) throw new Error('Los servicios base se agregan automáticamente al rentar');
    ...
    const ofrecido = ofertas.some((o) => {
      ...
      return String(nombreOferta).trim().toLowerCase() === String(servicio.nombre).trim().toLowerCase();
    });
    if (!ofrecido) throw new Error('El servicio no está ofrecido para esta unidad');

    const relacionExistente = await EstudianteUnidadServicio.findOne({
      where: { estudiante_unidad_id: estudianteUnidadId, servicio_id: servicioId }
    });
  ...
  async eliminarServicioDeAsignacion(estudianteUnidadId, servicioId) {
    ...
    if (!relacion) throw new Error('El servicio no está asociado a esta asignación');
